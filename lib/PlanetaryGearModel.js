...
class PlanetaryGear {
  ...
  fixed_speed_update(elapsed_time) {
    let factor = 0;
    // Update ring
    factor = ((elapsed_time / 1000) / ((1 / this.ring.gear_speed) * 2 * PI)) % 1;
    this.ring.position = 2 * PI * factor + this.ring.mesh_correcction;
    // Update sun
    factor = ((elapsed_time / 1000) / ((1 / this.sun.gear_speed) * 2 * PI)) % 1;
    this.sun.position = 2 * PI * factor;
    // Update planets
    for (let n = 0; n < this.planets.length; n++) {
      let planet = this.planets[n];
      factor = ((elapsed_time / 1000) / ((1 / planet.gear_speed) * 2 * PI)) % 1;
      planet.position = 2 * PI * factor + planet.mesh_correcction;
    }
    // Update carrier
    factor = ((elapsed_time / 1000) / ((1 / this.carrier_speed) * 2 * PI)) % 1;
